...
function extraUpdate(v, bins) {
    ...
    containers = dom.select('.mark-1').selectAll('rect');
    ...
    containers.on('mouseover', function (d, i) {
        ...
        if (stats.dragging.on) {
            ...
            if (i < stats.dragging.left) {
                ...
                for (j = i; j <= stats.dragging.left; j += 1) {
                    e = d3.select(containers[0][j]);
                    e.style('opacity', 0.3)
                        .style('fill', 'red')
                        .datum(select);
                }
                ...
                for (j = stats.dragging.right + 1; j <= i; j += 1) {
                    e = d3.select(containers[0][j]);
                    e.style('opacity', 0.3)
                        .style('fill', 'red')
                        .datum(select);
                ...
                if (stats.dragging.from === 0) {
                    ...
                    for (j = stats.dragging.left; j < i; j += 1) {
                        e = d3.select(containers[0][j]);
                        e.style('opacity', 0.0)
                            .style('fill', 'black')
                            .datum(unselect);
                    ...
                    for (j = i + 1; j <= stats.dragging.right; j += 1) {
                        e = d3.select(containers[0][j]);
                        e.style('opacity', 0.0)
                            .style('fill', 'black')
                            .datum(unselect);
    ...
    function mouseup(d, i) {
        ...
        binwidth = (stats.end - stats.start) / stats.bins;
        oldstart = stats.start;
                    ...
                    $.ajax({
                        ...
                        success: function (response) {
                            ...
                            d3.text("/lib/vgd3-template.js.txt", function (text) {
                                ...
                                d3.select("#quartile").classed("disabled", false).node().onclick = function () {
                                    static_histogram(stats.start, stats.end, 4, "#chart", true, stats.extraupdate_template);
                                };

                                d3.select("#decile").classed("disabled", false).node().onclick = function () {
                                    static_histogram(stats.start, stats.end, 10, "#chart", true, stats.extraupdate_template);
                                };

                                d3.select("#percentile").classed("disabled", false).node().onclick = function () {
