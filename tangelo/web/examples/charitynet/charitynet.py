...
def run(servername, dbname, datatype, by=None,
        datemin=None, datemax=None, charity=None):
    ...
    if datatype == "transactions":
        ...
        if by == "month":
            ...
            group = "$county"
        pipeline.append({"$group": {"_id": group,
                                    "amount": {"$sum": "$amount"}}})
        ...
        if by == "month":
            ...
            response = [["%05d" % d["_id"], float(d["amount"])]
                        for d in result["result"]
                        if d["_id"] is not None]
        ...
        coll = db["charitynet.normalized.transactions"]
        result = coll.aggregate([{"$group": {"_id": "$charity_id",
                                             "count": {"$sum": 1}}},
                                {"$sort": {"count": -1}}])
