...
QUnit.test("Non-required non-existing config file", function (assert) {
    ...
    tangelo.plugin.config.config("/data/doesntexist.yaml", false, function (config) {
        QUnit.start();
...
QUnit.test("Required non-existing config file", function (assert) {
    ...
    tangelo.plugin.config.config("/data/doesntexist.yaml", true, function (config, error) {
        QUnit.start();
        ...
        assert.strictEqual(error.file, "/data/doesntexist.yaml", "'error' parameter named the missing file");
    });
});
...
QUnit.test("Non-required existing config file", function (assert) {
    ...
    tangelo.plugin.config.config("/data/config.yaml", false, function (config) {
        QUnit.start();
...
QUnit.test("Required existing config file", function (assert) {
    ...
    tangelo.plugin.config.config("/data/config.yaml", true, function (config) {
        QUnit.start();
...
QUnit.test("default 'required' parameter value", function (assert) {
    ...
    tangelo.plugin.config.config("/data/doesntexist.yaml", function (config) {
        QUnit.start();

        assert.deepEqual(config, {}, "'required' defaults to false (non-existing config file)");
    });

    tangelo.plugin.config.config("/data/config.yaml", function (config) {
        QUnit.start();
